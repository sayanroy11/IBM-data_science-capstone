...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names='Launch Site',
        title='Successful launches based on Launch Site')
        return fig
    else:
        ...
        names='class',
        title='Success vs Failure for {}'.format(entered_site)
        )
        return fig
# TASK 4:
...
def get_scatter_plot(site,range):
    low,high=range
    ...
    if site == 'ALL':
        ...
        temp_df=spacex_df[spacex_df['Launch Site']==site]
        mask=(temp_df['Payload Mass (kg)'] > low) & (temp_df['Payload Mass (kg)'] < high)
        fig = px.scatter(temp_df[mask], x='Payload Mass (kg)',
        y='class',color='Booster Version Category',
        title='Payload mass VS Outcome for {}'.format(site)
        )
        return fig
